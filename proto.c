...
static int proto_append_encoded_text(char *target, const char *source, unsigned int capacity);
...
static unsigned long proto_crc32_n(const char *data, unsigned int length);
static unsigned long proto_crc32_step(unsigned long crc, const unsigned char *data, unsigned int length);
static int proto_find_crc_marker(const char *line);
static int proto_extract_crc(const char *line, char *crc_text, unsigned int capacity);
static int proto_verify_line(const char *line);
...
static long proto_extract_long(const char *line, const char *key, long default_value);
